 ...
 * 查找灯带光点
 */
void findByContours(Mat & image, vector < Point > &pointVector, vector < Point > &trapezoidPoints,
                    vector < Mat > &outMats) {
    // 将图像从BGR转换到HSV颜色空间
    cv::Mat hsv;
...
//    // 定义增强系数
//    double sFactor = 1.1; // 饱和度增强系数
    double vFactor = 1.1; // 亮度增强系数
//
//    // 创建S和V通道的LUT
    int lutSize = 256;
...
////    // 将HSV图像转换回BGR颜色空间以便显示
    ...
    Mat dst = thresholdPoints(gImg, hsvChannels[0], outMats);
    Mat dst2 = thresholdPoints(rImg, hsvChannels[0], outMats);
...
//           3, KMEANS_PP_CENTERS, centers);
    // 计算每个聚类中心到所有点的平均距离
//    vector<double> cluster_avg_distances(k, 0.0);
//    vector<int> cluster_points_count(k, 0);
//
//    for (int i = 0; i < pointVector.size(); i++) {
//        Point2f point = pointVector[i];
//        int cluster_idx = labels.at<int>(i, 0);
//        Point2f center = centers.at<Point2f>(cluster_idx);
//        double distance = sqrt(pow(point.x - center.x, 2) + pow(point.y - center.y, 2));
//        cluster_avg_distances[cluster_idx] += distance;
//        cluster_points_count[cluster_idx]++;
//    }
//
//    for (int i = 0; i < k; i++) {
//        cluster_avg_distances[i] /= cluster_points_count[i];
//        LOGD(LOG_TAG, "cluster_avg_distances i = %d i = %f cluster_points_count=%d", i,
//             cluster_avg_distances[i], cluster_points_count[i]);
//    }
//
//    // 检测离群点
...
//        Point2f center = centers.at<Point2f>(cluster_idx);
//        double distance = sqrt(pow(point.x - center.x, 2) + pow(point.y - center.y, 2));
//        double ratio = distance / cluster_avg_distances[cluster_idx];
//        if (ratio > stddevThreshold) {
//            circle(outMat, pointVector[i], 8, Scalar(0, 0, 0), 5);
...
//    }
    ...
    for (int i = 0; i < pointVector.size(); i++) {
        ...
        if (labels.at<int>(i) == 0) {
            ...
            circle(outMat, pointVector[i], 10, colorScalar, 5);
...
Mat thresholdPoints(Mat & src, Mat & hue, vector < Mat > &outMats) {
    ...
    while (contoursSize > 125 && thresh < 220) {
        threshold(src, threshold_image, thresh, 255, THRESH_BINARY);
        morphology_image = morphologyImage(threshold_image, 3, 5);
    ...
    // 定义腐蚀的结构元素
    Mat kernelErode = getStructuringElement(MORPH_ELLIPSE, Size(21, 21));
...
*/
Mat morphologyImage(Mat & image, int
openKernelSize,
...
putText(image, to_string(angle), point1, FONT_HERSHEY_SIMPLEX,
0.8,
Scalar(0, 0, 0), 2);
...
double angleSelect =
        abs(closestAngleRight) > abs(closestAngleLeft) ? abs(closestAngleRight) : abs(
                closestAngleLeft);
if (angleSelect <= 5) {
...
push_back(pointLeft);
trapezoid4Points.
push_back(Point2i(leftTopX, minY)
);
trapezoid4Points.
push_back(Point2i(rightTopX, minY)
);
trapezoid4Points.
push_back(pointRight);
